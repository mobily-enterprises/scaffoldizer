  ...
  for (const m of listOfManipulations) {
    switch (m.op) {
      ...
      case 'insert':
        ...
        spacesRegExp = new RegExp(`(.*)${m.anchorPoint}`)
        spacesMatched = contents.match(spacesRegExp, contents)
        if (spacesMatched && spacesMatched[1]) spaces = spacesMatched[1]

        if (m.position === 'before') value = `${value}${spaces}${m.anchorPoint}`
        else value = `${spaces}${m.anchorPoint}${value}`

        contents = contents.replace(new RegExp(`.*${regexpEscape(m.anchorPoint)}.*`), `${value}`)
        break
    }
  }
  return contents
