...
exports.walk = (dir, allFiles, prefixPath, exts) => {
  allFiles = allFiles || []
  ...
  for (const file of files) {
    ...
    if (fs.lstatSync(fullPath).isDirectory()) {
      exports.walk(fullPath, allFiles, path.join(prefixPath, file), exts)
    } else {
...
exports.injectPlaceholder = (template, file, textToBeAdded, insertBelow) => {
  ...
  if (!match) {
    console.error('WARNING!!!!!!! Could not find match!')
    return file
  }

  return insertFragment(file, match[1].length, textToBeAdded, insertBelow)
}
exports.file2Info = function (config, file, fileInfoFunction) {
  const contents = fs.readFileSync(path.join(config.dstDir, file)).toString()
  return fileInfoFunction(contents)
}

exports.getFiles = function (config, filter, fileInfoFunction = () => { return {} }, exts) {
  const allFiles = exports.walk(config.dstDir, null, null, exts)
  const matchingFiles = []

  for (const file of allFiles) {
    const contents = fs.readFileSync(path.join(config.dstDir, file)).toString()

    const info = exports.file2Info(config, file, fileInfoFunction)
  ...
  for (const m of listOfManipulations) {
    ...
    switch (m.op) {
      ...
      case 'insert':
        ...
        if (value.startsWith('import Projects')) debugger
        contents = exports.injectPlaceholder(anchorPoint, contents, value, m.insertBelow)
      ...
      case 'custom':
        ...
        contents = await f(contents, m, config, exports)
        break
      default:
