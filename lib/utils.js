...
export const prompt = async (question) => {
  ...
  try {
    switch (type) {
      case 'select':
        return await select({ message, choices: normalizeChoices(choices), default: initial })
      case 'multiselect':
        return await checkbox({ message, choices: normalizeChoices(choices), default: initial })
      case 'confirm':
      case 'toggle':
