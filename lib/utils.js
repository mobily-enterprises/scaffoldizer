...
import { input, select, confirm, checkbox, password, number } from '@inquirer/prompts'

const debug = debugLib('logs')

// Expose individual prompt fns if needed by custom code
export const promptFns = { input, select, confirm, checkbox, password, number }

export const prompt = async (question) => {
  const { type, message, choices, initial, validate } = question || {}
  // Default pagination size for list-like prompts
  const pageSize = typeof question?.pageSize === 'number'
    ? question.pageSize
    : (typeof question?.limit === 'number' ? question.limit : 15)
  try {
    switch (type) {
      case 'select':
        return await select({ message, choices: normalizeChoices(choices), default: initial, pageSize })
      case 'multiselect':
        return await checkbox({ message, choices: normalizeChoices(choices), default: initial, pageSize })
      case 'confirm':
      case 'toggle':
        return await confirm({ message, default: initial })
      case 'password':
        return await password({ message, default: initial })
      case 'number':
        return await number({ message, default: initial })
      case 'text':
      default:
        return await input({ message, default: initial, validate })
    }
  } catch (e) {
    // Unify cancel semantics with jskit: print and throw a friendly error
    const msg = e && e.message
    if (msg === 'User force closed the prompt with 0 null bytes') {
      console.error('Aborting...')
      const err = new Error('Operation aborted by user')
      err.code = 'E_PROMPT_ABORT'
      throw err
    }
    throw e
  }
...
// Back-compat: provide a `prompts(q)` function that resolves to { value }
export const prompts = async (q) => ({ value: await prompt(q) })

const normalizeChoices = (choices) => {
  if (!Array.isArray(choices)) return []
  return choices.map(c => ({ name: c?.name ?? c?.title ?? String(c?.value ?? ''), value: c?.value, disabled: c?.disabled }))
}
