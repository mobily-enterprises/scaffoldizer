...
exports.injectPlaceholder = (template, file, textToBeAdded) => {
  ...
  function countLastLineIndents(inputString) {
    ...
    const indentCount = lastLine.match(/^\s*/)[0].length;
    return indentCount
  }
  ...
  const templateRegex = new RegExp('(.*)(' + escapedTemplate + ')(.*)', 'sm');
  ...
  if (match) {
    ...
    const indents = countLastLineIndents(match[2])
    return match[1] + match[2] + indentStringBySpaces(textToBeAdded, indents) + match[4] + match[5];
  } else {
    return file;
  }
}
...
exports.getFiles = function (config, filter, fileInfoFunction = () => { return {} } ) {
  ...
  for (const file of allFiles) {
    ...
    if (typeof filter === 'function') {
      filterReturn = filter(contents, info); // Assigning value to filterReturn
...
exports.manipulateText = async function (config, contents, listOfManipulations) {
  ...
  debug('Running manupulations for', config.moduleJson5Values.name,'...')
